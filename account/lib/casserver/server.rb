...
module CASServer
  class Server < Sinatra::Base
    ...
    # START -DUCKSAN: Extra routes =================================================================================================
    post "#{uri_path}/direct_login" do
      ...
      if @username && settings.config[:downcase_username]
        $LOG.debug("[DC] Converting username #{@username.inspect} to lowercase because 'downcase_username' option is enabled.")
        @username.downcase!
      end
      
      # @lt = generate_login_ticket.ticket

      $LOG.debug("[DC] Logging in with username: #{@username}, service: #{@service}, auth: #{settings.auth.inspect}")
      ...
      begin
        ...
        $LOG.warn("[DC] Invalid credentials given for user '#{@username}'")
        @message = {:type => 'mistake', :message => _("Incorrect username or password.")}
        status 401
        # redirect @service+(URI.parse(@service).query ? "&" : "?")+"lt=#{@lt}"
        redirect @service
      end            
    ...
    # 2.2
    post "#{uri_path}/direct_login_link" do
      Utils::log_controller_action(self.class, params)
      # 2.2.1 (optional)
      @service = clean_service_url(params['service'])
      ...
      begin
        ...
        if credentials_are_valid
          ...
          $LOG.info("[DC] Credentials for username '#{@username}' successfully validated using #{successful_authenticator.class.name}.")
          $LOG.debug("[DC] Authenticator provided additional user attributes: #{extra_attributes.inspect}") unless extra_attributes.blank?

          # 3.6 (ticket-granting cookie)
          tgt = generate_ticket_granting_ticket(@username, extra_attributes)
          response.set_cookie('tgt', tgt.to_s)

          $LOG.debug("[DC] Ticket granting cookie '#{tgt.inspect}' granted to #{@username.inspect}")

          if @service.blank?
            $LOG.info("[DC] Successfully authenticated user '#{@username}' at '#{tgt.client_hostname}'. No service param was given, so we will not redirect.")
            ...
            begin
              ...
              $LOG.info("[DC] Redirecting authenticated user '#{@username}' at '#{@st.client_hostname}' to service '#{@service}'")
              redirect service_with_ticket, 303 # response code 303 means "See Other" (see Appendix B in CAS Protocol spec)
            rescue URI::InvalidURIError
              $LOG.error("[DC] The service '#{@service}' is not a valid URI!")
              @message = {
                :type => 'mistake',
